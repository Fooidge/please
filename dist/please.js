...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Color = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"./Color.coffee\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
	...
	var please = {};
	exports.default = please;


	please.Color = function (color) {
		return new _Color2.default(color);
	};

	var PHI = 0.618033988749895;

	var makeColorDefaults = {
		hue: null,
		saturation: null,
		value: null,
		base_color: '',
	...
	please.generateFromBaseColor = function (baseColor) {
		...
		var base = new _Color2.default(baseColor);
		color.hue(clamp(_lodash2.default.random(base.hue() - 5, base.hue() + 5), 0, 360));
		if (base.saturation() === 0) {
			color.saturation(0);
		} else {
			color.saturation(_lodash2.default.random(0.4, 0.85, true));
		}
		color.value(_lodash2.default.random(0.4, 0.85, true));
		return color;
	};

	please.generate = please.generateGolden = function () {
		var color = new _Color2.default();
		var hue = _lodash2.default.random(0, 359);
	...
	please.generateRandom = function () {
		var color = new _Color2.default();
		color.hue(_lodash2.default.random(0, 359));
		color.saturation(_lodash2.default.random(0, 1.0, true));
		color.value(_lodash2.default.random(0, 1.0, true));
		return color;
	};

	var deprecationLayer = function deprecationLayer(options) {
	...
	please.makeColor = function () {
		var options = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];

		//remove deprecationLayer after 3 months in the wild
		var opts = deprecationLayer(_lodash2.default.defaults(makeColorDefaults, options));
		var colors = [];
		var iterable = __range__(0, opts.colorsReturned, true);
		for (var j = 0; j < iterable.length; j++) {
			var i = iterable[j];
			colors[i] = please.generate();
			//remove overwrites after 3 months in the wild
			//overwrite values if option exists to
			if (opts.hue != null && _lodash2.default.isNumber(opts.hue)) {
				colors[i].hue(opts.hue);
			}
			if (opts.saturation != null && _lodash2.default.isNumber(opts.saturation)) {
				colors[i].saturation(opts.saturation);
			}
			if (opts.value != null && _lodash2.default.isNumber(opts.value)) {
				colors[i].value(opts.value);
			}
			switch (opts.format.toLowerCase()) {
				case 'hex':
					colors[i] = colors[i].hex();break;
				case 'rgb':
					colors[i] = colors[i].rgbString();break;
				case 'hsl':
					colors[i] = colors[i].hslString();break;
				default:
